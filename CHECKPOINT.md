...
| `checkpoint_id` | BIGINT, okunabilir ardışık id (`GENERATED BY DEFAULT AS IDENTITY`). PK DEĞİL, sadece göz ile takip için. |
| `checkpoint_no` | `solution_index/interval_size`, hesaplanmış kolon (`GENERATED ALWAYS ... STORED`). Her satır KENDİ `interval_size`'ına göre hesaplanır; ileride interval değişse bile eski satırlar bozulmaz. |
